...
# Configurations
ROOT_DIR = "/home/romain/StylizedMetaAlbum/stylized-meta-album/style_dataset_preparation/stylized_image_generation/"
VERBOSE = True
...
def main(input_dir, output_dir, submission_dir, data_name):
    ...
    try:
        import style_extraction
        import importlib
        importlib.reload(style_extraction)
        path.remove(submission_dir)
    except ImportError:
        print(f"experiment not found in {submission_dir}")
        exit(1)       

    style_extraction.run(output_dir, input_dir)
