...
def run(output_dir, input_dir):
    ...
    def extract_styles(style_image, outputs_path):
        s = Image.open(style_image)
        s_tensor = trans(s).unsqueeze(0).to(device)
        out = model.extact_style(s_tensor, alpha)
    ...
    output_stylized_dir = f"{output_dir}/{domain}/styles_features"

    cmpt = 0
    for i, style_path in enumerate(df.STYLE_PATH.unique()):
    	style_cat = style_path.split("/")[0]
    	style_name = style_path.split("/")[-1]
    	os.makedirs(f"{output_stylized_dir}/{style_cat}", exist_ok=True)
    	location_style_image = f"{task_dir}/styles/{style_path}"
    	location_output_style = f"{output_stylized_dir}/{style_cat}/{style_name}.pt"
    	out = extract_styles(location_style_image, location_output_style)
    	if i%100 == 0:
